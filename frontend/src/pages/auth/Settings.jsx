...
export default function Settings() {
  ...
  const onProfileSubmit = async (values) => {
    ...
    try {
      const updated = await userService.updateProfile({ name: values.name })
      setUser(updated)
      ...
      <motion.div variants={item}>
        <Section title="Profile">
          ...
          <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} className="space-y-3">
            ...
            {/* Email — read-only */}
            <div>
              <label className="text-xs text-text-muted mb-1.5 block">Email <span className="text-text-muted/60">(cannot be changed)</span></label>
              <div className="relative">
                <Mail size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted" />
                <input
                  value={user?.email || ''}
                  readOnly
                  className="input-field pl-9 text-sm opacity-50 cursor-not-allowed"
                />
              </div>
            </div>
